...
async function initGitIfNeeded(): Promise<void> {
  ...
  if (shouldInit) {
    ...
    execSync('git commit -m "Initial commit — generated by ArcadeForge"', {
      cwd: process.cwd(),
      stdio: 'pipe',
    });
...
async function deployNetlify(): Promise<void> {
  ...
  console.log('   2. Go to netlify.com → New site from Git');
  console.log('   3. Select your repo — Netlify will auto-detect the config');
}
...
async function submitToHub(snapshot: GameDesignSnapshot, gameUrl: string): Promise<void> {
  ...
  console.log(chalk.blue('   ✨ Submitting to ArcadeForge Hub...'));
  ...
  try {
    ...
    if (response.ok) {
      console.log(chalk.bold.green('   🎉 Game submitted! It will appear on the Hub after moderation.'));
    } else {
...
export async function runDeploy(): Promise<void> {
  ...
  const target = await select<DeployTarget>({
    ...
    choices: [
      { value: 'vercel', name: 'Vercel (Recommended — fastest one-click)' },
